...
#xdmf_disp.parameters["rewrite_function_mesh"] = False

# time marching
t = 0
time = np.linspace(0, t_final, Nsteps+1)
disp_value = np.zeros(Nsteps+1)
disp_value[0] = u_st.sub(1)(length,width/2.0, height/2.0)
...
for (i,dt) in enumerate(np.diff(time)):
	...
	disp_value[i+1] = u.sub(1)(length,width/2.0,height/2.0)

	update_fields(u, u_old, v_old, a_old)
	xdmf_disp.write(u,t)

np.savetxt("DispEnd.txt", np.transpose([time,disp_value]), delimiter=',')
plt.plot(time,disp_value)
plt.grid(True)
plt.xlabel("time (s)")
plt.ylabel("Displacement (m)")
plt.savefig("DispEnd.png")
